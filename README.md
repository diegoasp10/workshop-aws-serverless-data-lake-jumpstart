...
Todos los componentes y servicios utilizados en este taller, como Amazon S3, AWS Glue, Amazon Athena, y AWS Glue DataBrew, han sido provisionados utilizando AWS CDK (Cloud Development Kit). AWS CDK permite definir toda la infraestructura como código, facilitando la creación, configuración y eliminación de recursos de manera programática.

Para desplegar esta CDK utiliza los siguiente comando en la raíz de la cdk:

```bash
sudo chmod +x scripts/deploy.sh
script/deploy.sh <your_aws_profile>
```

Con esto s desplegaran los recursos necesarios para el workshop.
